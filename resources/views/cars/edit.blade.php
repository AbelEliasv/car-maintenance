                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('cars.edit', $car) }}"
                            class="ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-400 dark:hover:text-white">Editar</a>
                    </div>
                </li>
            </ol>
        </nav>
        <form method="POST" action="{{ route('cars.update', $car) }}">
            ...
            <div class="mb-6">
                <label for="marca" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    Marca</label>
                <input type="text" id="text" name="brand" value="{{ $car->brand }}"
            ...
            <div class="mb-6">
                <label for="marca" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    Modelo</label>
                <input type="text" id="text" name="model" value="{{ $car->model }}"
            ...
            <div class="mb-6">
                <label for="año" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    Año</label>
                <input type="text" id="text" name="year" value="{{ $car->year }}"
            ...
            <div class="mb-6">
                <label for="owner" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Select an
                    Propietario</label>
                <select id="owner" name="owner_id"
                    ...
                    @foreach ($owners as $owner)
                        <option value="{{ $owner->id }}"
                            {{ $owner->id == $car->owner_id ? 'selected' : '' }}>{{ $owner->name }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="text-right ">
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Guardar
                    cambios</button>
            </div>
